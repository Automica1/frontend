      ...
      <div className="max-w-7xl mx-auto relative z-10">
        ...
        <div className="grid lg:grid-cols-3 gap-8">
          {values.map((value, index) => (
            ...
            >
              <div className={`relative h-full p-8 lg:p-10 rounded-3xl backdrop-blur-xl transition-all duration-500 group-hover:transform group-hover:scale-105 overflow-hidden ${
                hoveredIndex === index 
                ...
                <div className="relative z-10">
                  <div className="mb-8">
                    <div className={`w-18 h-18 rounded-2xl flex items-center justify-center transition-all duration-500 relative ${
                      hoveredIndex === index 
                        ? 'bg-gradient-to-br from-purple-600 to-purple-500 shadow-lg shadow-purple-500/25'
                        : 'bg-gray-900/50 border border-gray-800/50'
                    }`}>
                      <value.icon className={`w-9 h-9 transition-all duration-500 ${
                        hoveredIndex === index 
                          ? 'text-white' 
                          : 'text-purple-400'
                      }`} />
                      ...
                      <div className={`absolute -top-1 -right-1 w-3 h-3 rounded-full transition-all duration-500 ${
                        hoveredIndex === index ? 'bg-purple-400 animate-pulse' : 'bg-gray-700'
                      }`}></div>
                    </div>
                  </div>
