...
export const ProcessingActionCard: React.FC<ProcessingActionCardProps> = ({
  ...
  const getStatusMessage = () => {
    ...
    if (solutionType === 'face-verify') {
      if (files.length === 1) {
        return 'Upload one more face image to verify';
      }
      if (files.length === 2) {
  ...
  const hasFeedbackSlot = Boolean(feedbackSlot);

  const requirementsBlock = (
    <>
      {solutionType === 'signature-verification' && (
        <div className={`bg-blue-900/20 border border-blue-500 rounded-lg ${hasFeedbackSlot ? 'p-2.5' : 'p-3'}`}>
      ...
      {solutionType === 'face-verify' && (
        <div className={`bg-blue-900/20 border border-blue-500 rounded-lg ${hasFeedbackSlot ? 'p-2.5' : 'p-4'}`}>
          <div className="flex items-center space-x-2 mb-1">
            <div className="w-3 h-3 bg-blue-500 rounded-full" />
            <h4 className="text-sm font-semibold text-blue-400">Requirements</h4>
          </div>
  ...
  return (
    <div className="bg-gray-900 rounded-lg border border-gray-700 h-full flex flex-col overflow-hidden">
      <div className={`flex-1 min-h-0 overflow-y-auto p-4 pb-0 ${hasFeedbackSlot ? 'space-y-2' : 'space-y-3'}`}>
        ...
        {!hasFeedbackSlot && requirementsBlock}

        {hasFeedbackSlot && (
          <div className="flex flex-col min-h-0 gap-2">
            {requirementsBlock}
            <div className="min-h-0 flex-1">{feedbackSlot}</div>
          </div>
        )}
      </div>
